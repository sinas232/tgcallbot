...
if [[ ! -f constants.py ]] || ! grep -q '^BOT_VERSION = "2.3.21"$' constants.py; then
  echo "Wrong source tree: expected the v2.3.21 source tree. Do not deploy an older checkout." >&2
  exit 2
fi
...
docker compose exec -T bot python -c 'from constants import BOT_VERSION; assert BOT_VERSION == "2.3.21", BOT_VERSION; print("Bot checkout version:", BOT_VERSION)'
docker compose ps

"""اعتبارسنجی لینک سفارش — حالت private برای لینک دعوت، any برای سازگاری.
...
# ─── تنظیمات ────────────────────────────────────────────────────────────
def link_mode() -> str:
    """Mode is explicit; keep the pre-existing permissive default for upgrades."""
    raw = (os.getenv("ORDER_LINK_MODE") or getattr(Config, "ORDER_LINK_MODE", "any")).strip().lower()
    if raw not in ("private", "any"):
        raise ValueError("ORDER_LINK_MODE must be private or any")
    return raw
...
def validate_order_link(raw: Any, extra_urls: Iterable[Any] = ()) -> Tuple[bool, str, str]:
    ...
    """
    # Entity URLs may be an iterator; keep them for both candidate extraction
    # and selection of the actual clickable target in permissive mode.
    extra_urls = tuple(extra_urls or ())
    candidates = _link_candidates(raw, extra_urls)
    if not candidates:
        return False, "", message("empty")

    custom = (os.getenv("ORDER_LINK_REGEX") or getattr(Config, "ORDER_LINK_REGEX", "")).strip()
    if custom:
        # Never silently fall back to an open mode for a malformed restriction.
        try:
            compiled = re.compile(custom)
        except re.error as exc:
            raise ValueError("invalid ORDER_LINK_REGEX") from exc
        # The displayed text of a text_link is not the destination. Apply
        # custom policy to any explicitly visible invite first, then the
        # clickable entity URL, before falling back to legacy free text.
        preferred = ([c for c in _link_candidates(raw) if _match_invite(c)]
                     + _link_candidates("", extra_urls) + candidates)
    ...
    if link_mode() == "any":
        # A Telegram text_link displays an arbitrary label; joining that label
        # is impossible. Prefer the explicitly visible invite, then the actual
        # clickable URL, then legacy free text. Do not silently discard the
        # hidden target when ORDER_LINK_MODE keeps permissive checkout.
        visible = _scrub(raw)
        hidden = next((url for url in map(_scrub, extra_urls) if url), "")
        value = visible if invite_hash(visible) else hidden or visible or candidates[0]
        return True, normalize_invite_link(value) if invite_hash(value) else value, ""
...
def target_key(raw: Any) -> str:
    """Compare old and new invite spellings without folding the invite hash.

    Telegram invite hashes are case-sensitive. Lowercasing them would merge
    distinct groups, potentially blocking a purchase or leaving the wrong one.
    Public links remain distinct from private invites.
    """
    digest = invite_hash(raw)
    if digest is not None:
        return "invite:" + digest
    return "text:" + _scrub(raw).rstrip("/").casefold()

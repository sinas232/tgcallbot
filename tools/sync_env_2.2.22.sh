#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────────────
# همگام‌سازی .env با موتور جدید نسخهٔ ۲.۲.۲۲
# (سفارش با هر تعداد اکانت کامل اجرا می‌شود؛ تنها محدودیت = ۵ سفارش هم‌زمان)
#
#   bash tools/sync_env_2.2.22.sh            # اعمال تغییرات (با پشتیبان‌گیری)
#   bash tools/sync_env_2.2.22.sh --dry-run   # فقط نمایش تغییرات
# ─────────────────────────────────────────────────────────────────────────────
set -euo pipefail

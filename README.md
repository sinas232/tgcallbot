...
| [`docs/full-version.fa.md`](docs/full-version.fa.md) | موجودی قابلیت‌های کد ۲.۳.۲۳ و محدودیت‌های بررسی آفلاین |
| [`docs/env-compatibility.fa.md`](docs/env-compatibility.fa.md) | **گارد نام/مقدار تنظیمات تاریخی `.env`؛ پیش از استقرار بخوانید** |
| [`docs/deploy-final.fa.md`](docs/deploy-final.fa.md) | **دستورهای استقرار مشروط نسخهٔ ۲.۳.۲۳ روی نصب موجود**؛ بکاپ خصوصی/گارد پرداخت‌های pending پیش از Docker |
...
- **پنل مدیریت کامل:** مدیریت کاربران، سفارش‌ها، پلن‌ها، تیکت، بکاپ خودکار،
  ...
  مرحله‌ای و تک‌اکانتیِ سشن‌های غیرفعال، حذفِ پیش‌نمایش‌شدهٔ حساب‌های
  دلیت‌شده/سشن‌های باطل با پاسخ تأییدشدهٔ تلگرام، و مسیر **جداگانهٔ حذف
  آگاهانهٔ سشن‌های ۴۰۶ِ قرنطینه‌شده** بدون ادعای حذف خود حساب تلگرام.
  شمار «نامطمئن» به‌تنهایی مجوز حذف نیست: در ۲.۳.۱۹ علت‌هایش جدا گزارش می‌شوند؛
  بررسی پس از تداخل ۴۰۶/قطع نامطمئن یا سه خطای هم‌نوع پیاپی متوقف می‌شود؛
  ۴۰۶ `inactive` پس از قطع تأییدشده ثبت و به‌همراه نشانگرهای ۴۰۶ تاریخی از
  بررسی‌های *گروهی* بعدی کنار گذاشته می‌شود، نه اینکه حساب پاک شود؛
  پاسخ تایپ‌شدهٔ تلگرام **کلید را باطل می‌کند، نه حساب را**: با شمارهٔ همان ردیف
  دوباره وارد شوید. کلاینتِ بررسی و ورود تازه در حالت `USE_PROXY` از همان
  SOCKS5 کلاینت‌های ویس استفاده می‌کنند. **پس از دو ۴۰۶ تایپ‌شدهٔ تازه برای
  IDهای ۱۶ و ۱۷، ۲۲ کلید هنوز بررسی نشده‌اند؛ تکرار گروهی می‌تواند کلید بعدی
  را هم به خطر بیندازد.** کد ۲.۳.۲۱ تا جایگزینی کلیدهای قرنطینه‌شده با ورود
  شماره، کل بررسی گروهی را می‌بندد و چک اسپم زمان‌بندی‌شده را هنگام تعمیرات
  متوقف می‌کند. در ۲.۳.۲۲ سوپرادمین می‌تواند **صرفاً سشن‌های قرنطینه‌شدهٔ ۴۰۶**
  را با پیش‌نمایش و تأیید، بدون اتصال/OTP، از فهرست و DB حذف کند؛ این به معنی
  حذف حساب تلگرام نیست. قفل حادثه در DB باقی می‌ماند تا پس از حذف یا ورود
  تازه هم بررسی گروهی/تک‌اکانتی و چک اسپم خودکارِ کلیدهای قدیمی از سر گرفته
  نشود. طبق گزارش استقرار ۲.۳.۲۲، ۲۲ ردیف inactive ناشناخته باقی مانده‌اند.
  در ۲.۳.۲۳ سوپرادمینِ ربات اصلی می‌تواند با **انتخاب آگاهانهٔ جداگانه** و
  پیش‌نمایش دقیق همهٔ ۲۲ ID، بدون اتصال/OTP، کل ۲۲ کلید ذخیره‌شده را از DB
  کنار بگذارد؛ ممکن است بعضی سشن‌ها سالم باشند. این حذف خودکار نیست، حساب
  تلگرام را حذف نمی‌کند و قفل حادثه را باز نمی‌کند؛ وضعیت زنده/منشأ ۴۰۶
  با تست آفلاین اثبات نشده است.
...
# نسخهٔ ۲.۳.۱۹ به بعد تنظیمات تاریخی شناخته‌شده را با گارد نام/مقدار بررسی می‌کند.
...
- `main` هنوز روی ۲.۲.۳ است. کد شاخهٔ `arena/01a0ccf5-tgcallbot` نسخهٔ ۲.۳.۲۳ است؛ طبق گزارش کاربر، سرور ۲.۳.۲۲ را با بکاپ معتبر مستقر کرده و منو **۲۲ ردیف inactive، صفر نشانگر ۴۰۶ و قفل فعال** نشان داده است. تغییر شاخه یا ادغام GitHub، **سرور را خودکار به‌روز نمی‌کند**. مالک حذف آگاهانهٔ تمام ۲۲ ردیف باقی‌مانده را حتی با احتمال سالم‌بودن برخی سشن‌ها خواست؛ این گزینه در ۲.۳.۲۳ پس از استقرار، پیش‌نمایش و تأیید سوپرادمین عمل می‌کند. تا آن زمان آن ردیف‌ها هنوز حذف نشده‌اند. حذف کلید ذخیره‌شده به معنی حذف حساب تلگرام یا حل علت ۴۰۶ نیست و تست آفلاین سلامت سشن/تماس زنده را ثابت نمی‌کند.

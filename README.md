...
| [`docs/full-version.fa.md`](docs/full-version.fa.md) | موجودی قابلیت‌های کد ۲.۳.۲۱ و محدودیت‌های بررسی آفلاین |
| [`docs/env-compatibility.fa.md`](docs/env-compatibility.fa.md) | **گارد نام/مقدار تنظیمات تاریخی `.env`؛ پیش از استقرار بخوانید** |
| [`docs/deploy-final.fa.md`](docs/deploy-final.fa.md) | **دستورهای استقرار مشروط نسخهٔ ۲.۳.۲۱ روی نصب موجود**؛ قرنطینهٔ خصوصی فایل‌های ناشناس و ممیزی پرداخت‌های pending پیش از Docker |
...
- **پنل مدیریت کامل:** مدیریت کاربران، سفارش‌ها، پلن‌ها، تیکت، بکاپ خودکار،
  ...
  متوقف می‌کند؛ **حتی پس از ورود تازه هم بدون تعیین علت تداخل، گروهی را دوباره
  شروع نکنید.** تست آفلاین سلامت آن ۲۲ حساب را ثابت نمی‌کند.
...
- `main` هنوز روی ۲.۲.۳ است. کد شاخهٔ `arena/01a0ccf5-tgcallbot` نسخهٔ ۲.۳.۲۱ است؛ طبق گزارش اخیر، سرور نسخهٔ ۲.۳.۲۰ (commit `5a8e307`) را اجرا می‌کند. تغییر شاخه یا ادغام GitHub، **سرور را خودکار به‌روز نمی‌کند** و تست آفلاین سلامت سشن/تماس زنده را ثابت نمی‌کند. ردیف‌های ۱۳/۱۶/۱۷ با سشن فعلی قابل احیا نیستند و به ورود تازه با شماره نیاز دارند.

...
- **⚡️ حالت شنونده (Listener Mode):** اکانت‌ها به‌صورت پیش‌فرض **بدون
  انتشار رسانه** وارد تماس می‌شوند (بدون ffmpeg، بدون انکودر Opus) — مصرف هر
  اکانت تقریباً صفر است؛ در صورت نیاز خودکار به استریم سکوت برمی‌گردد.
- **🧠 بهداشت حافظه (RAM hygiene):** کلاینت‌های ویس‌کال فقط جریان raw
  updates را می‌گیرند (بدون fetch خودکار ریپلای/تاپیک/استوری — منبع طوفانِ
  `channels.GetMessages`)، کش‌ها و workerهای هر کلاینت کوچک‌اند، و یک
  **نگهبان خودکار (idle-client reaper)** هر کلاینتی که هیچ سفارشی به آن
  ارجاع نمی‌دهد را می‌بندد؛ به‌همراه خط لاگ دوره‌ای `[VoiceMemory]`
  (RSS، تعداد کلاینت/انجین، پروسه‌های ffmpeg) برای عیب‌یابی.

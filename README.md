...
| [`docs/full-version.fa.md`](docs/full-version.fa.md) | موجودی قابلیت‌های کد ۲.۳.۲۲ و محدودیت‌های بررسی آفلاین |
| [`docs/env-compatibility.fa.md`](docs/env-compatibility.fa.md) | **گارد نام/مقدار تنظیمات تاریخی `.env`؛ پیش از استقرار بخوانید** |
| [`docs/deploy-final.fa.md`](docs/deploy-final.fa.md) | **دستورهای استقرار مشروط نسخهٔ ۲.۳.۲۲ روی نصب موجود**؛ بکاپ خصوصی/ممیزی پرداخت‌های pending پیش از Docker |
...
- **پنل مدیریت کامل:** مدیریت کاربران، سفارش‌ها، پلن‌ها، تیکت، بکاپ خودکار،
  ...
  نشود. ۲۲ کلید نامعلوم دست‌نخورده‌اند؛ تست آفلاین سلامتشان را ثابت نمی‌کند.
...
- `main` هنوز روی ۲.۲.۳ است. کد شاخهٔ `arena/01a0ccf5-tgcallbot` نسخهٔ ۲.۳.۲۲ است؛ طبق خروجی تازهٔ کاربر، سرور ۲.۳.۲۱ را اجرا می‌کند و منو **سه نشانگر ذخیره‌شدهٔ ۴۰۶ برای ۱۳/۱۶/۱۷** نشان داده است. تغییر شاخه یا ادغام GitHub، **سرور را خودکار به‌روز نمی‌کند**؛ ۲.۳.۲۲ هنوز روی آن مستقر نشده و تست آفلاین سلامت سشن/تماس زنده را ثابت نمی‌کند. برای نگه‌داشتن این سه حساب ورود تازه با شماره لازم است، اما اگر سوپرادمین آن‌ها را نمی‌خواهد، مسیر تازهٔ حذف آگاهانهٔ سشن و ردیف بدون دسترسی به تلگرام فراهم است. حذف، خودِ حساب تلگرام یا ۲۲ کلید بررسی‌نشده را تغییر نمی‌دهد.
